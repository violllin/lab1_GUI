...
## Структура AST
### Описание узлов 
LetNode - узел объявления константы, содержит имя переменной и саму лямбду-функцию.

LambdaNode - тело функции, хранит список параметров, возвращаемый тип и само выражение.

ParamNode - описывает аргумент функции, содержит токен имени и токен типа.

BinOpNode - бинарная операция, связывает левый и правый операнды через оператор.

VarNode - использование переменной, хранит токен идентификатора для поиска в таблице символов.

LiteralNode - константное значение, на этом этапе проверяется вхождение чисел в диапазон 32-битного Int.
